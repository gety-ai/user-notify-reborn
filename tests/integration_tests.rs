...
fn test_notification_from_non_main_thread() {
    ...
    let handle = thread::spawn(move || {
        ...
        let test_result = rt.block_on(async {
            ...
            let manager = match NotifyManagerFactory::new(
            ...
            match manager.first_time_ask_for_notification_permission().await {
                Ok(permission) => {
                    ...
                    if !permission {
                        println!("Warning: Notification permission not granted, but continuing test");
                    }
                }
                Err(e) => {
            ...
            match manager.send(notification).await {
                Ok(handle) => {
                    println!("Notification sent successfully from thread with ID: {}", handle.get_id());
        ...
        let handle = thread::spawn(move || {
            ...
            let result = rt.block_on(async {
                let manager = match NotifyManagerFactory::new(
                    ...
                    Ok(m) => m,
                    Err(e) => {
                        eprintln!("Thread {}: Failed to create manager: {}", i, e);
                        return false;
                    }
                };
                ...
                if let Err(e) = manager.register(
                    Box::new(move |response| {
                        println!("Thread {}: Received response: {:?}", i, response);
                    }),
                    vec![],
                ) {
                    eprintln!("Thread {}: Failed to register handler: {}", i, e);
                    return false;
                }
                
                // send notification
                let notification = NotifyBuilder::new()
                    .title(&format!("Thread {} Notification", i))
                    .body(&format!("This is from thread number {}", i))
                    .subtitle("Concurrent Test");
                
                match manager.send(notification).await {
                    Ok(handle) => {
                        println!("Thread {}: Notification sent with ID: {}", i, handle.get_id());
                    ...
                    Err(e) => {
                        eprintln!("Thread {}: Failed to send notification: {}", i, e);
                        false
                    }
                }
            });
...
async fn test_async_spawn_notification() {
    ...
    let task_result = tokio::spawn(async move {
        ...
        match manager.send(notification).await {
            Ok(handle) => {
                println!("Async spawn: Notification sent with ID: {}", handle.get_id());
